...
void Mario::update(const unsigned i_view_x, MapManager& i_map_manager){
	//We make Mario bounce after updating all the enemies.
	...
	if (0 == dead){
		...
		//map_collision function returns a vector of numbers. Each number is a binary representation of the collisions in a single row. And we're storing that vector in this vector.
		vector<unsigned char> collision;

		//map_collision function can also return the coordinates of cells intersecting the hitbox. We store that too.
		vector<sf::Vector2i> cells;
		...
		if (0 == crouching){
			if (0 == sf::Keyboard::isKeyPressed(sf::Keyboard::Right) &&
				...
				horizontal_speed = max(horizontal_speed - MARIO_ACCELERATION, -MARIO_WALK_SPEED);
			}
			...
			if (0 == sf::Keyboard::isKeyPressed(sf::Keyboard::Left) &&
				...
				horizontal_speed = min(MARIO_ACCELERATION + horizontal_speed, MARIO_WALK_SPEED);
			}
		}
		...
		if (0 == moving){
			if (0 < horizontal_speed)
			{
				horizontal_speed = max<float>(0, horizontal_speed - MARIO_ACCELERATION);
			}
			else if (0 > horizontal_speed)
			{
				horizontal_speed = min<float>(0, MARIO_ACCELERATION + horizontal_speed);
			}
		}
		...
		if (0 < powerup_state){
			if (1 == sf::Keyboard::isKeyPressed(sf::Keyboard::C) || 1 == sf::Keyboard::isKeyPressed(sf::Keyboard::Down)){
				...
				if (1 == all_of(collision.begin(), collision.end(), [](const unsigned char i_value){
					return 0 == i_value;
				})){
					crouching = 0;
		...
		if (0 == all_of(collision.begin(), collision.end(), [](const unsigned char i_value){
			return 0 == i_value;
		})){
			moving = 0;
		...
		if (1 == sf::Keyboard::isKeyPressed(sf::Keyboard::Up) || 1 == sf::Keyboard::isKeyPressed(sf::Keyboard::Z)){
			if (0 == vertical_speed && 0 == all_of(collision.begin(), collision.end(), [](const unsigned char i_value){
				return 0 == i_value;
			})){
				...
				vertical_speed = min(GRAVITY + vertical_speed, MAX_VERTICAL_SPEED);
			}
		}else{
			vertical_speed = min(GRAVITY + vertical_speed, MAX_VERTICAL_SPEED);
