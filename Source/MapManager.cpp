...
void MapManager::draw_map(const bool i_draw_background, const bool i_underground, const unsigned i_view_x, sf::RenderWindow& i_window){
	...
	//Drawing the coin before drawing the blocks because we want it to appear behind the question block.
		...
		for (unsigned short b = 0; b < map_height; b++){
			...
			if (1 == i_draw_background){
				...
				if (Cell::Coin == map[a][b]){
					...
					//Since the underground blocks have a different look, their texture is placed 2 cells below the regular ones in the map texture.
...
void MapManager::update_map_sketch(const unsigned char i_current_level){
	map_sketch.loadFromFile("Resources/Images/LevelSketch" + to_string(static_cast<unsigned short>(i_current_level)) + ".png");
}
...
vector<unsigned char> MapManager::map_collision(const vector<Cell>& i_check_cells, const sf::FloatRect& i_hitbox) const{
	...
	for (short a = floor(i_hitbox.top / CELL_SIZE); a <= floor((ceil(i_hitbox.height + i_hitbox.top) - 1) / CELL_SIZE); a++){
		...
		for (short b = floor(i_hitbox.left / CELL_SIZE); b <= floor((ceil(i_hitbox.left + i_hitbox.width) - 1) / CELL_SIZE); b++){
			if (0 <= b && b < map.size()){
				if (0 <= a && a < map[0].size()){
					if (i_check_cells.end() != find(i_check_cells.begin(), i_check_cells.end(), map[b][a])){
			...
			else if (i_check_cells.end() != find(i_check_cells.begin(), i_check_cells.end(), Cell::Wall)){
				output[a - floor(i_hitbox.top / CELL_SIZE)] += pow(2, floor((ceil(i_hitbox.left + i_hitbox.width) - 1) / CELL_SIZE) - b);
			}
		}
	}
...
vector<unsigned char> MapManager::map_collision(const vector<Cell>& i_check_cells, vector<sf::Vector2i>& i_collision_cells, const sf::FloatRect& i_hitbox) const
{
	vector<unsigned char> output;
