...
void convert_sketch(const unsigned char i_current_level, unsigned short& i_level_finish, vector<shared_ptr<Enemy>>& i_enemies, sf::Color& i_background_color, MapManager& i_map_manager, Mario& i_mario){
	...
	//We divide the height by 3. The sketch stores the level as 3 layers: Blocks, Entities, & Background Tiles.
		...
		for (unsigned short b = 0; b < 2 * map_height; b++){
			...
			if (b < map_height){
				...
				if (sf::Color(255, 0, 0) == pixel){
					...
					i_enemies.push_back(make_shared<Goomba>(sf::Color(0, 0, 85) == i_background_color, CELL_SIZE * a, CELL_SIZE * (b - map_height)));
				}else if (sf::Color(0, 219, 0) == pixel){
					i_enemies.push_back(make_shared<Koopa>(sf::Color(0, 0, 85) == i_background_color, CELL_SIZE * a, CELL_SIZE * (b - map_height)));
				}
			}
		}
	}
